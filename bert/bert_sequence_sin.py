...
class BertSeqSinModel(nn.Module):

    # 初始化类
    def __init__(self, class_size, pretrained_name="bert-base-uncased"):
        """
        Args:
            :param class_size       :指定分类模型的最终类别数目，以确定线性分类器的映射维度
            :param pretrained_name  :用于指定bert的预训练模型
        """
        # 类继承的初始化，固定写法
        super(BertSeqSinModel, self).__init__()
        # 加载HuggingFace的BertModel
        # BertModel的最终输出维度默认为768
        # return_dict=True 可以使用BertModel的输出具有dict属性，即以bert_output['last_hidden_state']方式调用
        self.bert = BertModel.from_pretrained(pretrained_name, return_dict=True)

        # 通过一个线性层将[CLS]标签对应的维度：768->class_size
        # class_size 在SST-2情感分类任务重设置为：2
    ...
    def forward(self, inputs):
        ...
        # 将三者输入进模型，如果想知道模型内部如何运作，以后再来探索吧~
...
def load_sin_sure_data(data_path, train_ratio=0.8):
...
batch_size = 8
num_epoch = 5  # 训练轮次
check_step = 1  # 用以训练中途对模型进行检验：每check_step个epoch进行一次测试保存模型
data_path = "./data_sequence_sin.tsv"  # 数据所在地址
train_ratio = 0.8  # 训练集比例
learning_rate = 1e-5  # 优化器的学习率

# 获取训练、测试数据、分类类别总数
train_data, test_data, categories = load_sin_sure_data(data_path=data_path, train_ratio=train_ratio)
...
# 创建模型 BertSeqSinModel
model = BertSeqSinModel(len(categories), pretrained_model_name)
# 固定写法，将模型加载到device上，
# 如果是GPU上运行，此时可以观察到GPU的现存增加
model.to(device)
# 加载预训练模型对应的tokenizer
...
for epoch in range(1, num_epoch + 1):
    ...
    for batch in tqdm(test_dataloader, desc=f"Testing"):
        ...
        # 这个代码块中r的全部有关tensor的操作都不产生梯度。目的是节省时间和空间，不加也没事
        with torch.no_grad():
            bert_output = model(inputs)
            """
    ...
    if epoch % check_step == 0:
        # 保存模型
        checkpoints_dirname = "bert_sequence_sin_" + timestamp
